    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = (const CBlockIndex*) 0;
    BOOST_FOREACH(const PAIRTYPE(int64, uint256)& item, vSortedByTimestamp)
    {
        ...
        const CBlockIndex* pindex = mapBlockIndex.at(item.second);
		...
		unsigned int nTimeTx, uint256& hashProofOfStake, bool fPrintProofOfStake, StakeStats *stakeStats)
{
    ...
    if (nTimeBlockFrom + STAKE_MIN_AGE > nTimeTx) // Min age requirement
...
{
    assert (pindex->pprev || pindex->GetBlockHash() == GENESIS_HASH);
